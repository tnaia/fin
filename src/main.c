/*
  ...
  Copyright (C) 2012-2014  Bob Mottram <bob@robotics.uk.to>
...
int extract_ssh_port(char * server, char * server_name)
{
    int sctr=0, ctr=0, i, ssh_port = 22;
    char str[STRING_BLOCK];


    for (i = 0; i < strlen(server); i++) {
        if ((server[i] == ':') && (i < strlen(server)-1)) {
            if (server[i+1] != '/') {
                server_name[sctr++] = server[i];
                i++;
                while (i < strlen(server)-1) {
                    if (server[i] == '/') {
                        str[ctr++] = 0;
                        break;
                    }
            ...
            if (server!=0) {
                ...
                if (username != 0) {
                    sprintf((char*)command_str,
                            "scp -P %d -r -o User=%s %s %s",
                            ssh_port, username,
                            (char*)fin_directory, server_name);
                }
                else {
                    sprintf((char*)command_str,"scp -P %d -r %s %s",
                            ssh_port,
                            (char*)fin_directory, server_name);
                }
