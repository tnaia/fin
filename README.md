...
A subtlety of restoring from backup is that the compressed file contains the full path, including /home/username, so if you attempt to restore on a system with a different username from when the backup was created this will fail and you will need to extract the files mannually into your ~/.fin directory.
